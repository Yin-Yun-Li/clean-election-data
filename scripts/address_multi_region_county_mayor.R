...
df <- fread("C:\\Users\\user\\Desktop\\github-local-repo\\clean-election-data\\data\\tmp_county_mayor_dt.csv",
             encoding = "UTF-8")
...
dt <- dt %>% group_by(year, winning_party) %>% 
  filter(HSN_NM == "苗栗縣" & 
           TOWN_NM == "卓蘭鎮" & 
  ...
  summarize(HSN_NM = "苗栗縣", 
            TOWN_NM = "卓蘭鎮", 
            VILL_NM = "西坪里、坪林里", 
            county_total_vote = sum(county_total_vote), 
            county_eligible_vote = sum(county_eligible_vote),
            county_kmt_vote = sum(county_kmt_vote),
            county_dpp_vote = sum(county_dpp_vote),
            county_ruling_vote = sum(county_ruling_vote),
            .groups = "drop") %>% 
  setDT() %>% 
  rbind( dt[!(HSN_NM == "苗栗縣" & 
                TOWN_NM == "卓蘭鎮" & 
...
taichung_multi_region <- taichung_multi_region[, .( county_total_vote = sum(county_total_vote), 
                                                    county_eligible_vote = sum(county_eligible_vote),
                                                    county_kmt_vote = sum(county_kmt_vote),
                                                    county_dpp_vote = sum(county_dpp_vote),
                                                    county_ruling_vote = sum(county_ruling_vote)), 
                                            by = .( HSN_NM, TOWN_NM, VILL_GROUP, year, winning_party)]
...
nantou_multi_region <- nantou_multi_region[, .( county_total_vote = sum(county_total_vote), 
                                                county_eligible_vote = sum(county_eligible_vote),
                                                county_kmt_vote = sum(county_kmt_vote),
                                                county_dpp_vote = sum(county_dpp_vote),
                                                county_ruling_vote = sum(county_ruling_vote)), 
...
tainan_multi_region <- tainan_multi_region[, .( county_total_vote = sum(county_total_vote), 
                                                county_eligible_vote = sum(county_eligible_vote),
                                                county_kmt_vote = sum(county_kmt_vote),
                                                county_dpp_vote = sum(county_dpp_vote),
                                                county_ruling_vote = sum(county_ruling_vote)), 
...
taitung_multi_region <- taitung_multi_region[, .( county_total_vote = sum(county_total_vote), 
                                                  county_eligible_vote = sum(county_eligible_vote),
                                                  county_kmt_vote = sum(county_kmt_vote),
                                                  county_dpp_vote = sum(county_dpp_vote),
                                                  county_ruling_vote = sum(county_ruling_vote)), 
...
hualien_multi_region <- hualien_multi_region[, .( county_total_vote = sum(county_total_vote), 
                                                  county_eligible_vote = sum(county_eligible_vote),
                                                  county_kmt_vote = sum(county_kmt_vote),
                                                  county_dpp_vote = sum(county_dpp_vote),
                                                  county_ruling_vote = sum(county_ruling_vote)), 
...
bamboo_multi_region <- bamboo_multi_region[, .( county_total_vote = sum(county_total_vote), 
                                                county_eligible_vote = sum(county_eligible_vote),
                                                county_kmt_vote = sum(county_kmt_vote),
                                                county_dpp_vote = sum(county_dpp_vote),
                                                county_ruling_vote = sum(county_ruling_vote)), 
...
pingtung_multi_region <- pingtung_multi_region[, .( county_total_vote = sum(county_total_vote), 
                                                    county_eligible_vote = sum(county_eligible_vote),
                                                    county_kmt_vote = sum(county_kmt_vote),
                                                    county_dpp_vote = sum(county_dpp_vote),
                                                    county_ruling_vote = sum(county_ruling_vote)), 
...
lienchiang_multi_region <- lienchiang_multi_region[, .( county_total_vote = sum(county_total_vote), 
                                                        county_eligible_vote = sum(county_eligible_vote),
                                                        county_kmt_vote = sum(county_kmt_vote),
                                                        county_dpp_vote = sum(county_dpp_vote),
                                                        county_ruling_vote = sum(county_ruling_vote)), 
...
fwrite(dt[,.(HSN_CD,HSN_NM,TOWN_NM,VILL_NM,year,county_total_vote,county_eligible_vote,
             county_kmt_vote, county_dpp_vote, county_ruling_vote, winning_party)],
       "C:\\Users\\user\\Desktop\\github-local-repo\\clean-election-data\\data\\final_county_mayor_dt.csv",
       encoding = "UTF-8",
       bom = T)